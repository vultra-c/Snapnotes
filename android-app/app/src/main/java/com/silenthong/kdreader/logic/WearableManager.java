...
public class WearableManager {
     ...
     *
     * If addListener fails (e.g. because a previous listener wasn't fully
     * removed yet), we retry once after explicitly removing the old listener.
     * If the retry also fails, we check whether the error indicates an
     * already-registered listener — in that case we treat it as success.
     */
    private void registerMessageListener(final String deviceName) {
        ...
        messageApi.addListener(nodeId, new OnMessageReceivedListener() {
            ...
            public void onFailure(Exception e) {
                ...
                Log.w(TAG, "addListener failed: " + msg + " — retrying after remove");

                // First check common "already registered" patterns
                ...
                // Retry: explicitly remove the old listener, then add again
                retryRegisterAfterRemove(deviceName, nodeId, 1);
            }
        });
    }

    /**
     * Detect whether an error message indicates the listener is already registered.
     * The XMS SDK may return different message formats across versions.
     */
    private boolean isAlreadyRegisteredError(String msg) {
        if (msg == null || msg.isEmpty()) return false;
        String lower = msg.toLowerCase();
        return lower.contains("you have registered")
    ...
    /**
     * Retry listener registration after explicitly removing any existing listener.
     * @param deviceName device name for callback
     * @param nodeId target node ID
     * @param remainingRetries number of retries left
     */
    private void retryRegisterAfterRemove(final String deviceName,
                                          final String nodeId,
        ...
        messageApi.removeListener(nodeId)
                .addOnSuccessListener(aVoid -> {
                    if (destroyed || currentNode == null) return;
                    Log.d(TAG, "Old listener removed, retrying addListener...");
                    // Small delay to let the SDK settle
                    handler.postDelayed(() -> {
                        if (destroyed || currentNode == null) return;
                        doAddListener(deviceName, nodeId, remainingRetries);
                    }, 500);
                })
                .addOnFailureListener(e -> {
                    if (destroyed || currentNode == null) return;
                    Log.w(TAG, "removeListener before retry failed: " + e.getMessage());
                    // Try adding anyway
                    handler.postDelayed(() -> {
                        if (destroyed || currentNode == null) return;
                        doAddListener(deviceName, nodeId, remainingRetries);
    ...
    /**
     * Actually call addListener — used by both initial registration and retries.
     */
    private void doAddListener(final String deviceName,
                               final String nodeId,
                               final int remainingRetries) {
        messageApi.addListener(nodeId, new OnMessageReceivedListener() {
            @Override
            ...
            if (destroyed || currentNode == null) return;
            Log.d(TAG, "Message listener registered (retry) — connected!");
            connected = true;
            notifyConnected(deviceName);
        }).addOnFailureListener(e -> {
            if (destroyed) return;
            String msg = e.getMessage() != null ? e.getMessage() : "";
            Log.w(TAG, "addListener retry failed: " + msg);
            ...
            if (remainingRetries > 0) {
                Log.w(TAG, "Retrying... (" + remainingRetries + " attempts left)");
                handler.postDelayed(() -> {
                    if (destroyed || currentNode == null) return;
                    retryRegisterAfterRemove(deviceName, nodeId, remainingRetries - 1);
                }, 1000);
            } else {
                // All retries exhausted — treat as connected anyway.
                // The listener might still be active from a previous session,
                // and message routing will work if any listener is registered.
                Log.w(TAG, "All retries exhausted — assuming connected with existing listener");
                connected = true;
                notifyConnected(deviceName);
            }
        });
