...
import com.silenthong.kdreader.logic.FormulaPngRenderer;
...
public class MainActivity extends Activity {
    ...
    private FormulaPngRenderer formulaRenderer;

    // Loading dialog (for formula rendering/transfer and other long operations)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initViews();
        initWearableManager();
        formulaRenderer = new FormulaPngRenderer(this);
     ...
     * Used for formula rendering, formula transfer, and other long operations.
     */
    private void showLoadingDialog(String message) {
        ...
        loadingDialogText = new TextView(this);
        loadingDialogText.setText(message);
        loadingDialogText.setTextSize(TypedValueDimension(R.dimen.text_body));
        loadingDialogText.setTextColor(getColor(R.color.text_body));
        LinearLayout.LayoutParams tvParams = new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        tvParams.setMarginStart(dpToPx(16));
        loadingDialogText.setLayoutParams(tvParams);
        layout.addView(loadingDialogText);
    ...
    /**
     * Update the loading dialog message (called from any thread).
     */
    private void updateLoadingDialog(final String message) {
        mainHandler.post(() -> {
    ...
    private void onSendClick() {
        ...
        try {
            JSONObject parsed = new JSONObject(selectedFileContent);
            if (hasFormulasInJson(parsed)) {
                jsonData = parsed;
            }
        } catch (Exception e) {
            // 非 JSON 内容，直接作为纯文本发送
        ...
        if (jsonData != null) {
            // 含公式：先在后台渲染并传输公式图，再发送文本
            showLoadingDialog("正在准备公式...");
            final JSONObject json = jsonData;
            new Thread(() -> processFormulasAndSend(json, fileName),
                    "FormulaRenderer").start();
        } else {
            // 无公式：直接发送文本
            doSendTxtFile(fileName, selectedFileContent);
        }
    }

    /**
     * 检查 JSON 内容中是否含有 formulas 字段。
     */
    private boolean hasFormulasInJson(JSONObject json) {
        ...
        while (keys.hasNext()) {
            ...
            try {
                ...
                for (int i = 0; i < entries.length(); i++) {
                    ...
                    JSONArray formulas = entry.optJSONArray("formulas");
    ...
    /**
     * 后台线程：渲染公式 PNG → 传输到手环 → 更新 JSON 中的 formulaImages → 发送文本。
     */
    private void processFormulasAndSend(JSONObject json, String fileName) {
        int processed = 0;
        int failed = 0;
        ...
        try {
            ...
            while (keys.hasNext()) {
                ...
                JSONArray entries;
                try {
                    entries = json.getJSONArray(subject);
                } catch (Exception e) {
                    continue;
                }
                ...
                for (int i = 0; i < entries.length(); i++) {
                    ...
                    int entryId = entry.optInt("id", i + 1);

                    // 收集 LaTeX 字符串
                    List<String> latexList = new ArrayList<>();
                    for (int f = 0; f < formulas.length(); f++) {
                        String latex = formulas.optString(f, "");
                        if (!latex.isEmpty()) {
                            latexList.add(latex);
                        }
                    }
                    if (latexList.isEmpty()) continue;

                    updateLoadingDialog("正在渲染公式: " + subject + " #" + entryId);

                    // 渲染为 PNG（同步阻塞）
                    FormulaPngRenderer.PngResult png = formulaRenderer.render(latexList);
                    if (png == null) {
                        Log.w(TAG, "公式渲染失败: " + subject + "#" + entryId + "，跳过");
                        failed++;
                        continue;
                    }

                    updateLoadingDialog("正在传输公式图: " + subject + " #" + entryId);

                    // 传输到手环（同步阻塞）
                    final boolean[] ok = {false};
                    txtTransfer.sendFormula(subject, entryId, png.bytes, png.width, png.height,
                            new TxtTransferHandler.TransferProgressListener() {
                                ...
                                @Override
                                public void onError(String error) {
                                    Log.e(TAG, "公式传输失败: " + error);
                                }
                            });

                    if (ok[0]) {
                        // 更新 JSON：为每条公式添加对应的 formulaImage 文件名
                        String formulaFileName = txtTransfer.formulaFileName(subject, entryId);
                        JSONArray formulaImages = new JSONArray();
                        for (int f = 0; f < formulas.length(); f++) {
                            formulaImages.put(formulaFileName);
                        }
                        entry.put("formulaImages", formulaImages);
                        processed++;
                    } else {
                        failed++;
                    }
            ...
            Log.i(TAG, "公式处理完成: 成功=" + processed + " 失败=" + failed);
        } catch (Exception e) {
            Log.e(TAG, "公式处理异常", e);
        }

        final String updatedContent = json.toString();
        ...
        mainHandler.post(() -> {
            dismissLoadingDialog();
            if (finalProcessed > 0) {
                tvProgress.setText("公式图已传输 " + finalProcessed + " 张" +
                        (finalFailed > 0 ? "（" + finalFailed + " 张失败）" : ""));
            }
            doSendTxtFile(fileName, updatedContent);
        });
    }

    /**
     * 发送文本内容到手环（常规 TXT 传输流程）。
     */
    private void doSendTxtFile(String fileName, String content) {
        final int charCount = content.length();
        txtTransfer.sendTxtFile(fileName, content, targetFolderId,
                new TxtTransferHandler.TransferProgressListener() {
                    @Override
                    public void onProgress(int sent, int total, String message) {
                        mainHandler.post(() -> {
